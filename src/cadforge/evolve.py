...
def evolve(db_path=DEFAULT_DB,output_dir='artifacts/continual',cloud=False):
    ...
    try:
        reusable=service.apply('supported_fastener',probe,context=ctx)
    except KeyError:
        ...
        for i,(d,c) in enumerate([(2.,.1),(2.,.3),(4.,.1),(4.,.3),(3.,.2)]):
            ...
            ids.append(service.run_experiment(_case(task,f'bore-{i}',p),boundary_bore,stage='discovery',context=ctx))
        bore=service.propose_affine('clearance_bore','bore_diameter',('fastener_diameter','radial_clearance'),ids)
        for stage,points in [('counterexample',[(3.5,.15,1)]),('transfer',[(2.5,.2,1),(3.,.25,2)])]:
            ...
            service.validate_candidate(bore,cases,verify,stage=stage,context=ctx)
        service.promote(bore);learned.append(bore)
        ...
        for i,(d,c,w) in enumerate([(2.,.1,1.),(2.,.1,2.),(4.,.3,1.),(4.,.3,2.),(3.,.2,1.5)]):
            p=service.apply('clearance_bore',{'fastener_diameter':d,'radial_clearance':c,'min_wall':w,'bore_diameter':d,'boss_outer_diameter':16.,'family_code':0.},context=ctx)
            p['boss_outer_diameter']=round(p['bore_diameter']+.2,8)
            ids.append(service.run_experiment(_case(task,f'boss-{i}',p),boundary_boss,stage='discovery',context=ctx))
        boss=service.propose_affine('supported_fastener','boss_outer_diameter',('bore_diameter','min_wall'),ids,parent_ids=(bore,))
        for stage,points in [('counterexample',[(3.5,.15,1.25,1)]),('transfer',[(2.5,.15,1.5,1),(3.,.25,1.75,2)])]:
            ...
            service.validate_candidate(boss,cases,verify,stage=stage,context=ctx)
        service.promote(boss);learned.append(boss)
        reusable=service.apply('supported_fastener',probe,context=ctx)
    ...
    service.validate_candidate(service.latest_id('supported_fastener',context=ctx),[monitor_case],verify,stage='monitor',context=ctx)
