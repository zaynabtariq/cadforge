...
out=Path('artifacts/axial-height-browser')
...
assert abs(after.extents[2]-before.extents[2]-10)<1e-5
assert baseline.read_bytes()==(out/'undone.stl').read_bytes()
assert all(c['passed'] for c in pv['checks'])
result={'holes':len(features),'protected_vertices':len(indices),'height_increase_mm':float(after.extents[2]-before.extents[2]),
        'original_protected_vertices_exact':True,'undo_bytes_exact':True,
        'usage':record['result']['usage'],'scope':'Public rotated plate, one protected height edit; no strength qualification or new learned-rule promotion.'}
