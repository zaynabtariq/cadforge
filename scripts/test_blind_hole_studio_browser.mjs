...
const output=path.join(root,'artifacts','blind-hole-studio-test');
...
try{
 ...
 const xy=await page.evaluate(()=>window.cadforge.projectPoint([60,70,12.7]));
 ...
 assert.equal(summary.edit.preview.command.op,'drill_blind_hole');
...
near=c.vertices[np.linalg.norm(c.vertices[:,:2]-entry[:2],axis=1)<=radius+1e-4]
assert len(near)>0
actual_depth=entry[2]-near[:,2].min()
