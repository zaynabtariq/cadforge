...
const out=path.resolve('artifacts/axial-sequence-browser');
...
try {
 ...
 await page.locator('#file-input').setInputFiles(path.resolve('artifacts/axial-browser/rotated-public-plate.stl'));
 ...
 await page.locator('#prompt').fill('Make this 1 cm wider then make this 1 cm wider, keeping holes fixed throughout');
 ...
 if(report.result.preview.step_outcomes.length!==2)throw Error('Expected two actual steps');
 for(const step of report.result.preview.step_outcomes)if(!step.checks.some(c=>c.name==='invariant.preserve_holes'&&c.passed))throw Error('Missing hole invariant');
