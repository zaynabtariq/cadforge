...
const output=path.join(root,'artifacts','learning-preview-browser');await fs.mkdir(output,{recursive:true});
...
const context=await browser.newContext({baseURL:'http://127.0.0.1:2740',viewport:{width:1600,height:1000},acceptDownloads:true,recordVideo:{dir:path.join(output,'raw'),size:{width:1600,height:1000}}});
...
try{
 await page.goto('http://127.0.0.1:2740',{waitUntil:'networkidle'});
...
print(json.dumps({'points':v.tolist(),'target_min':v.min(axis=0).tolist(),'target_max':v.max(axis=0).tolist()}))`],{encoding:'utf8'}));
  ...
  record.visible_summary=await page.locator('#preview-summary').innerText();
  if(!record.visible_summary.includes('tapering toward its boundary'))throw Error('Preview omitted actual transition semantics');
  if(index===0){
