...
const output=path.join(root,'artifacts','learning-video');await fs.mkdir(output,{recursive:true});
...
finally{const video=page.video();await context.close();if(video){videoPath=path.join(output,selectionOnly?'selection-diagnostic.webm':'actual-learning-flow.webm');await fs.copyFile(await video.path(),videoPath);}await browser.close();const result={selectionOnly,records,errors,failure,video:videoPath,scope:'Actual public development UI interactions; no store reset, API mocks, injected selection or hidden benchmark data.'};await fs.writeFile(path.join(output,selectionOnly?'selection-diagnostic.json':'actual-learning-result.json'),JSON.stringify(result,null,2));console.log(JSON.stringify({failure,errors,records:records.map(r=>({step:r.step,model:r.model,selection:r.selection.region,target:r.target,accepted:r.response?.preview?.accepted,learning:r.response?.preview?.learning}))},null,2));}
