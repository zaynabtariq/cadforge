...
const output=path.join(root,'artifacts','project-studio-test');
...
try{
 ...
 await page.locator('#prompt').fill('Move this .5 cm right');
 ...
 assert.equal(summary.edit.preview.command.op,'translate');
 ...
 const savedSession=await page.evaluate(()=>window.cadforge.getState().id);
 const projectEvent=page.waitForEvent('download',{timeout:30000});
 await page.locator('#save-project').click();
 const projectDownload=await projectEvent;
 const projectPath=path.join(output,'saved-project.cadforge');
 await projectDownload.saveAs(projectPath);
 const importEvent=page.waitForResponse(r=>new URL(r.url()).pathname==='/api/import',{timeout:30000});
 await page.locator('#file-input').setInputFiles(projectPath);
 const reopened=await (await importEvent).json();
 assert.notEqual(reopened.id,savedSession,'Project import must create a new independent session');
 await page.locator('#busy').waitFor({state:'hidden'});
 await download('reopened.stl');
 assert.deepEqual(await fs.readFile(path.join(output,'reopened.stl')),await fs.readFile(path.join(output,'committed.stl')),'Portable project must retain exact committed export');
 summary.project={archive:projectPath,original_session:savedSession,reopened_session:reopened.id,history_entries:reopened.history.length,exact_reopened_export:true};
 await page.screenshot({path:path.join(output,'04-reopened.png')});
 ...
 await page.screenshot({path:path.join(output,'05-reopened-undo.png')});
 await page.locator('#objects button').first().click();
 await page.locator('#prompt').fill('Move this 1 cm left');
 const nextResponse=page.waitForResponse(r=>/\/api\/sessions\/[^/]+\/edit$/.test(new URL(r.url()).pathname),{timeout:120000});
 await page.locator('#send').click();
 summary.continued_edit=await (await nextResponse).json();
 assert.ok(summary.continued_edit.preview?.accepted,'Reopened project must accept a new ordinary-language edit');
 await page.locator('#preview-banner').waitFor({state:'visible'});
 await page.locator('#busy').waitFor({state:'hidden'});
 await page.locator('#apply').click();
 await page.locator('#preview-banner').waitFor({state:'hidden'});
 await page.locator('#busy').waitFor({state:'hidden'});
 await download('continued-edit.stl');
 await page.screenshot({path:path.join(output,'06-continued-edit.png')});
...
assert np.allclose(c.bounds,b.bounds+[5,0,0],atol=2e-5),(b.bounds,c.bounds)
assert np.allclose(c.extents,b.extents,atol=2e-5)
continued=trimesh.load(p/'continued-edit.stl',force='mesh')
assert np.allclose(continued.bounds,b.bounds+[-10,0,0],atol=2e-5),'New edit on reopened project failed'
...
print(json.dumps({'translation_mm':[5,0,0],'baseline_bounds':b.bounds.tolist(),'committed_bounds':c.bounds.tolist(),'preview_did_not_commit':True,'undo_exact_export':True,'watertight':bool(c.is_watertight)}))
